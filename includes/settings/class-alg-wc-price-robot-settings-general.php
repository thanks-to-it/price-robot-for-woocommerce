...
class Alg_WC_Price_Robot_Settings_General extends Alg_WC_Price_Robot_Settings_Section {
	 ...
	 * @since   1.0.0
	 *
	 * @todo    (v2.0.0) `get_orders_manual`: add nonce
	 * @todo    (desc) `alg_wc_price_robot_all_products`
	 * @todo    (desc) Data update
	 */
	function get_settings() {
		$settings = array(
			...
			array(
				'desc_tip'          => (
					__( '"Automatic pricing" option must be enabled.', 'price-robot-for-woocommerce' ) . ' ' .
					sprintf(
						/* Translators: %s: Products page link. */
						__( 'If disabled, visit WooCommerce %s to enable the robot on per product basis.', 'price-robot-for-woocommerce' ),
						'<a href="' . admin_url( 'edit.php?post_type=product' ) . '">' .
							__( 'products page', 'price-robot-for-woocommerce' ) .
						'</a>'
					)
				),
			...
			array(
				...
				'desc_tip'          => sprintf(
					/* Translators: %s: Products page link. */
					__( 'Adds "Price Robot Debug" column to WooCommerce %s.', 'price-robot-for-woocommerce' ),
					'<a href="' . admin_url( 'edit.php?post_type=product' ) . '">' .
						__( 'products page', 'price-robot-for-woocommerce' ) .
					'</a>'
				),
			...
			array(
				...
				'desc'              => (
					__( 'Schedule', 'price-robot-for-woocommerce' ) . '<br>' .
					'<a href="' . add_query_arg( 'get_orders_manual', 'yes' ) . '">' .
						__( 'Update data now', 'price-robot-for-woocommerce' ) .
					'</a>'
				),
			...
			array(
				'desc'              => (
					__( 'Timeframe in days', 'price-robot-for-woocommerce' ) . '<br>' .
					__( 'If you modify this timeframe, then data must be updated for prices to change. This will be done automatically, according to the "Schedule" option.', 'price-robot-for-woocommerce' ) . '<br>' .
					sprintf(
						/* Translators: %s: URL. */
						__( 'You can also <a href="%s">update data manually</a>.', 'price-robot-for-woocommerce' ),
						add_query_arg( 'get_orders_manual', 'yes' )
					) . ' ' .
			...
			array(
				...
				'desc'              => __( 'Enable', 'price-robot-for-woocommerce' ),
				'desc_tip'          => (
					__( 'Treat variations as single product.', 'price-robot-for-woocommerce' ) . ' ' .
