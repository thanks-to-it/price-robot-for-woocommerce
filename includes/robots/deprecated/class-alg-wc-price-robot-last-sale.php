...
class Alg_WC_Price_Robot_Last_Sale extends Alg_WC_Price_Robot_Abstract {
	...
	function get_price( $price, $product_id, $original_price ) {
		...
		if ( 0 != ( $min_stock = get_option( 'alg_price_robot_last_sale_min_stock', 0 ) ) ) {
			...
			$product_stock = (
				alg_wc_price_robot()->core->is_wc_version_below_3 ?
	...
	function get_settings() {
		$settings = array(
			...
			array(
				...
				'desc'              => (
					__( 'If you modify this timeframe - data must be updated for prices to change. This will be done automatically, according to settings in General section.', 'price-robot-for-woocommerce' ) . '<br>' .
					sprintf(
						/* Translators: %s: URL. */
			...
			array(
				...
				'desc'              => sprintf(
					/* Translators: %1$.2f: Average number of products, %2$d: Number of days. */
					__( 'Sales in your shop: on average %1$.2f pieces of each product are sold in %2$d days timeframe.', 'price-robot-for-woocommerce' ),
